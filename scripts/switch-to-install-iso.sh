...
# ==============================================================================
# Script: scripts/switch-to-install-iso.sh
# Purpose: Dynamically detect CD-ROM target from VM XML and switch media
#          to redstar-install.iso for package installation.
# ==============================================================================
...
run_on_host() {
  echo "=== Switching VM '${VM_NAME}' media to Install ISO ==="
  ...
  # Dynamically determine CD-ROM target device from XML
  TARGET=""
  ...
  echo "Inserting Install ISO (${INSTALL_ISO}) into '${TARGET}'..."
  virsh change-media "${VM_NAME}" "${TARGET}" "${INSTALL_ISO}" --insert --force

  echo "=== Successfully inserted Install ISO ==="
...
if [ -e /etc/os-release ] && grep -qi "ubuntu" /etc/os-release && which virsh >/dev/null 2>&1; then
  ...
  scp "${SSH_OPTS[@]}" "${BASH_SOURCE[0]}" ubuntu@"${EC2_IP}":/tmp/switch-to-install-iso.sh
  ssh "${SSH_OPTS[@]}" ubuntu@"${EC2_IP}" "chmod +x /tmp/switch-to-install-iso.sh && sudo /tmp/switch-to-install-iso.sh"
fi
