...
# ==============================================================================
# Script: scripts/eject-redstar-iso.sh
# Purpose: Dynamically detect CD-ROM target from VM XML and eject any
#          mounted media so the VM boots purely from the qcow2 virtual disk.
# ==============================================================================
...
run_on_host() {
  echo "=== Ejecting CD-ROM media from VM '${VM_NAME}' ==="
  ...
  echo "Ejecting media from '${TARGET}'..."
  virsh change-media "${VM_NAME}" "${TARGET}" --eject --force 2>/dev/null || true

  echo "=== Successfully ejected CD-ROM media ==="
  echo "Current block devices for '${VM_NAME}':"
...
if [ -e /etc/os-release ] && grep -qi "ubuntu" /etc/os-release && which virsh >/dev/null 2>&1; then
  ...
  scp "${SSH_OPTS[@]}" "${BASH_SOURCE[0]}" ubuntu@"${EC2_IP}":/tmp/eject-redstar-iso.sh
  ssh "${SSH_OPTS[@]}" ubuntu@"${EC2_IP}" "chmod +x /tmp/eject-redstar-iso.sh && sudo /tmp/eject-redstar-iso.sh"
fi
